...
@section('title', __('My bundles') . ' — ' . config('app.name'))
@section('heading', __('My bundles'))

@section('content')
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-white">{{ __('My bundles') }}</h1>
        <a href="{{ route('agent.bundles.create') }}" class="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-[#0f0f1a]">{{ __('New bundle') }}</a>
    </div>
            ...
            <tbody>
                @foreach ($bundles as $b)
                    <tr class="border-b border-white/5">
                        ...
                        <td class="px-3 py-2">
                            <a href="{{ route('agent.bundles.edit', $b) }}" class="text-emerald-400 hover:underline">{{ __('Edit') }}</a>
                            <span class="text-slate-600">|</span>
                            <a href="{{ route('agent.bundles.resale-plans.index', $b) }}" class="text-emerald-400/90 hover:underline">{{ __('Resale') }}</a>
                            <form method="post" action="{{ route('agent.bundles.availability', $b) }}" class="inline">
                                @csrf
                                @method('PATCH')
