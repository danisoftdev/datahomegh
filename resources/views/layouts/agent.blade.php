    ...
    <div id="agent-app" class="flex min-h-screen" x-data="{ sidebarOpen: false }">
        ...
        >
            ...
            <nav class="min-h-0 flex-1 space-y-1 overflow-y-auto p-3 text-sm">
                ...
                <a href="{{ route('agent.orders.index') }}" class="{{ $r->routeIs('agent.orders.*') ? 'bg-white/10 text-emerald-400' : 'text-slate-300 hover:bg-white/5' }} block rounded-lg px-3 py-2">{{ __('My orders') }}</a>
