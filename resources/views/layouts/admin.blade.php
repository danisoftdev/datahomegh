...
<head>
    ...
    @include('partials.favicon-links', ['href' => \App\Support\BrandingFavicon::urlForAdminSession(auth()->user())])
    ...
    <div id="admin-app" class="flex min-h-screen" x-data="{ sidebarOpen: false }">
        ...
        >
            ...
            <nav class="min-h-0 flex-1 space-y-1 overflow-y-auto p-3 text-sm">
                ...
                <a href="{{ route('admin.fulfillment-apis.index') }}" class="{{ $r->routeIs('admin.fulfillment-apis.*') ? 'bg-white/10 text-[#FFD700]' : 'text-slate-300 hover:bg-white/5' }} block rounded-lg px-3 py-2">{{ __('Data APIs') }}</a>
                ...
                <a href="{{ route('admin.withdrawals.index') }}" class="{{ $r->routeIs('admin.withdrawals.*') ? 'bg-white/10 text-[#FFD700]' : 'text-slate-300 hover:bg-white/5' }} block rounded-lg px-3 py-2">{{ __('Agent withdrawals') }}</a>
