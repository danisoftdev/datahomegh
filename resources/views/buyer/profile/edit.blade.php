...
@section('content')
    ...
    <div class="mt-8 max-w-2xl space-y-4 rounded-xl border border-red-500/30 bg-red-500/5 p-6">
        ...
        <p class="text-sm text-slate-400">{{ __('This permanently removes your account, wallet, and order history from this platform. You can register again later with the same username, email, or phone.') }}</p>
        <form method="post" action="{{ route('buyer.profile.destroy') }}" class="space-y-4" onsubmit="return confirm(@json(__('Permanently delete your account? This cannot be undone.')))">
            @csrf
            <div>
